...
// TestAsk_EmptyTwice documents that Ask gives up after one retry: with
// both attempts empty, callers get (nil, nil) and must supply their own
...
func TestAsk_EmptyTwice(t *testing.T) {
	...
	if d != nil {
		t.Fatalf("expected nil decision after two empty responses, got %+v", d)
	}
	if calls != 2 {
